...
def s4_dog():
    ...
    """s4-dog"""
    x = range(len(names))
    mean1 = list(pd.read_csv("ACC_source.csv")["Accuracy"])
    var1 = list(pd.read_csv("ACC_source.csv")["char"])
    ...
    """CSTA"""
    mean2 = list(pd.read_csv("ACC_source_del_t7.csv")["Accuracy"])
    var2 = list(pd.read_csv("ACC_source_del_t7.csv")["char"])
